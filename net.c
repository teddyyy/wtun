...
static void encap_tunnel_udp_header(struct sk_buff* skb, 
				int udplen, u32 srcip, u32 dstip)
{
   	...
   	udph->dest   = htons(dst_port);
    	...
    	udph->check = 0;
    	udph->check = csum_tcpudp_magic(srcip, dstip, udplen, IPPROTO_UDP, 
				csum_partial(udph, udplen, 0));
}

static void encap_tunnel_ip_header(struct sk_buff* skb, 
				int iplen, u32 srcip, u32 dstip)
{
...
static void encap_tunnel_eth_header(struct sk_buff* skb, u8* srcmac, u8* dstmac)
{
